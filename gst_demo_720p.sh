#!/bin/bash
gst-launch-1.0 -v --gst-debug-level=1 v4l2src device=/dev/video0 ! "video/x-raw,width=1280,height=720,format=(string)UYVY" ! nvvidconv ! "video/x-raw(memory:NVMM),width=1280,height=720,format=(string)I420" ! nvoverlaysink sync=false
